...
chrome.extension.onRequest.addListener(function(Request, sender, sendResponse) {
	...
	switch (Request.reqType) {
		...
		case "removeClickedMerId":
			...
			break;
		case "getSeNumCoupon":
			Ajax.getUrlCoupon(Request.tagUrl, tabId, false, true, function(Data){
				sendResponse(Data);
			});
			break;
		case "addFavorite":
		...
		case "getFavorites":
			...
			break;
		case "snoozePush":
			Ajax.snoozeFavorite(tabUrl, tabId, Request.merId, function(Data){
				sendResponse(Data);
			});
			break;
		case "logErrorXpath":
		...
		case "turnOnPush":
			...
			break;
		case "turnOffPush":
			var ret	= Storage.savePushStatus("off");
			if (ret) Init.clearIntevalPush();
			var retVal	= ret ? 0 : -1;
			sendResponse({ret: retVal});
			break;
		default:
...
var Init = {
	...
	pushCoupon: function() {
		Mix.log("Push coupon");
		if (Storage.getPushStatus() == "off") return false;
		Init.clearIntevalPush();
		
		chrome.tabs.getSelected(null, function(tab){
			if (!tab.url || tab.url.indexOf("http") !== 0) { return false;}
