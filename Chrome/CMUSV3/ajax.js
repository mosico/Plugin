var Ajax = {
	...
	getUrlCoupon: function(tabUrl, tabId, winId, isLog, isSearch, callback) {
		var Param		= this.getParam(tabUrl, tabId);
		...
		if (winId > 0) {
			chrome.windows.get(winId, function(Win){
				...
				Ajax.request(REQUEST_COUPON_URL, this.postType, Param, this.jsonData, callback);
			});
		} else {
			Ajax.request(REQUEST_COUPON_URL, this.postType, Param, this.jsonData, callback);
		}
	...
	snoozeFavorite: function(tabUrl, tabId, merId, snoozeTime, callback) {
		var Param	= this.getParam(tabUrl, tabId);
		Param.merId	= merId;
		Param.snoozeTime = snoozeTime;
