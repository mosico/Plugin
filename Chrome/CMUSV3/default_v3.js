...
//		}
	...
	initApplyInfo: function() {
		...
		Context.applyInfo	= {count: count, codeIndex: 0, domain: "", status: "start", isApply: false, applyCode: '', codeLink: '',
...
//		Context.reqData.pageRegular.appendXpath	= ['//div[@id="h-f-2col-innerbody-wrapper"]/div[@class="h-f-2col-main"]/div[@class="h-f-2col-right"]'];
	...
	execute: function() {
		...
		function loopTry() {
			...
			if (loopTimes <= maxLoopTimes) Context.callCodeUrl(curCoupon.link);
			Context.setEndTime();
...
//			Context.applyInfo.finalPrice	= minPrice > 0 ? Math.min(minPrice, price) : price;
			...
			if (loopTimes === 1) {
				...
				Context.callCodeUrl(Context.applyInfo.codeLink);
			...
			if (removeDom) {
				...
				Context.applyInfo.useInfo.push({couponId: curCoupon.couponId, code: curCoupon.code, price: price, reduce: 0, 
												isRemove: false, startTime: curTime, link: curCoupon.link});
	...
	getBestCode: function() {
		...
		try {
			...
			if (useInfo[0].reduce > 0 && useInfo[0].price <= Context.applyInfo.originalPrice) {
				code	= useInfo[0].code;
				Context.applyInfo.codeLink	= useInfo[0].link;
	...
	callCodeUrl: function(url) {
		if (url && url.length > 0) {
			$("#cmus2012_callLink").attr("src", url);
		}
	},
...
var Html = {
	buildPopup: function(innerHtml) {
		var html	= '\
			...
			<div class="cmus_iframe"><iframe src="" style="display:none" id="cmus2012_callLink"></iframe></div>\
						';
		return html;
	},
